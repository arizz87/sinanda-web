...
function tambahdata()
		...
		check_session()
			...
			$.ajax({
			url: "modul/mod_aplikasi/prosesdaftartahun.php",
			data: "mode="+mode,
			cache: false,
...
check_session()
	$.ajax({
	type: "POST",
	url: "modul/mod_aplikasi/cruddaftartahun.php",
	dataType: 'json',
	data: {id_tahun:id_tahun,type:"get"},
	success: function(data) {  
	$("#myModalsdok").modal("show");
	$("#modedok").val("dokumen-editdata")
	var mode = $("#modedok").val() 
	$("#kodedok").val(data.id_tahun)
...
check_session(); 
	...
	var statuss = $("#status").val();   
				if(tahun==''){
				  toastr.error('Maaf Anda belum mengisi Tahun Anggaran .. !');
				}else if(namakepala==''){
				  toastr.error('Maaf Anda belum mengisi Nama Kepala Instansi .. !');
				}else if(nip==''){
				  toastr.error('Maaf Anda belum mengisi NIP Kepala Instansi .. !');
				}else if(statuss==''){
				  toastr.error('Maaf Anda belum mengisi Status Aktif .. !');
				}else{ 	
				var idupdate = $("#idupdate").val();  		
				  ...
				  let formData = new FormData();
					...
					$.ajax({
			            type: 'POST',
			            url: "modul/mod_aplikasi/simpan_tahun.php",
			            data: formData,
			            cache: false,
			            processData: false,
			            contentType: false,
	...
	var statuss = $("#status").val();   
				...
				var idupdate = $("#idupdate").val();  		
				  ...
				  let formData = new FormData();
					...
					$.ajax({
			            type: 'POST',
			            url: "modul/mod_aplikasi/simpan_tahun2.php",
			            data: formData,
			            cache: false,
			            processData: false,
			            contentType: false,
...
function submitHapus()
			{
				  var idhapus = $("#idhapus").val(); 			
						$.ajax({
							url: "modul/mod_aplikasi/hapus_datatahun.php",
