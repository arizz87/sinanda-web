...
//print_r($_SESSION);
	...
	*/
	$aColumns = array( 'id_dukung', 'id_data', 'id_data','tri1','tri2','tri3','tri4','posting'); //Kolom Pada Tabel
	
	// Indexed column (used for fast and accurate table cardinality)
	$sIndexColumn = 'id_dukung';
		
	// DB table to use
	$sTable = 'tb_datadukung_bidang'; // Nama Tabel
	//Koneksi Database
	...
	*/
	...
	while ( $aRow = $rResult->fetch_assoc() ) {
		$row = array(); 
		$no++;
		$btnaprove= '<a href="#" onClick="aktif(\''.$aRow['id_dukung'].'\')"><button type="button" title="Aktifkan Triwulan" class="btn btn-primary btn-xs"><i class="fa fa-check-square"></i> Triwulan</button></a> 
		<a href="#" onClick="hapus	(\''.$aRow['id_dukung'].'\')"><button type="button" title="Hapus Data" class="btn btn-danger btn-xs"><i class="fa fa-trash"></i> Hapus</button></a>';
		...
		//$aRow['nousul']
		$row = array('<center>'.$no,'( <font style="font-size:14px;">'.$cari1['kode_data'].' ) '.$cari1['uraian_data'], $tri1, $tri2, $tri3, $tri4, '<center>'.$btnaprove);
		$output['aaData'][] = $row;
 }
//}
